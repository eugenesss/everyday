...
class EventInformation extends Component { 
    ...
    renderNonEditField = (info) => {
      return (
        <div>
          <Row form>
            <Col md={6} style={{marginTop: 0, paddingLeft: 10, paddingRight: 15, position:'relative'}}>
    ...
    renderEditableField = (info) => {
      return (
        <div>
          <Row form>
            <Col md={6} style={{marginTop: 0, paddingLeft: 10, paddingRight: 15, backgroundColor:'rgba(0,0,0,0.1)', borderRadius: 8}}>
              <KeyboardDateTimePicker
                margin="normal"
                id="mui-pickers-time"
                label="Time picker"
                value={info.start}
                onChange={e => {this.editField("start", e._d)}}
                KeyboardButtonProps={{
                  'aria-label': 'change time',
                }}
              />
            ...
            <Col md={6} style={{marginTop: 0, paddingLeft: 10, paddingRight: 15, backgroundColor:'rgba(0,0,0,0.1)', borderRadius: 8}}>
              <KeyboardDateTimePicker
                ...
                label="Time picker"
                value={info.end}
                onChange={e => { this.editField("end", e._d)}}
                KeyboardButtonProps={{
                  'aria-label': 'change time',
                }}
              />
          ...
          <Row form>
            <TextField
              style={{ marginTop:15, width: '100%', backgroundColor:'rgba(0,0,0,0.1)', borderRadius: 8}}
              value={info.title}
              required
          ...
          <TextField
            ...
            // className={classes.textField}
            onChange={e => {this.editField("desc", e.target.value)}}
            id="description"
            label="Description"
            margin="normal"
            variant="outlined"
    ...
    render() {
        ...
        return (
          <Form>
            ...
            {!this.state.edit &&
              this.renderNonEditField(this.state.info)
            }
