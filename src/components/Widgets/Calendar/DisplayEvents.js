    ...
    render() {
        ...
        if(myEvents.length > 0){
            ...
            EventList = myEvents.map((item, index) => {
                ...
                return (
                    <div key={index} style={{
                        ...
                        borderRadius: 15,
                        padding: 21, marginBottom: 25,
                        boxShadow: '0 4px 8px 0 rgba(0, 0, 0, 0.08), 0 6px 20px 0 rgba(0, 0, 0, 0.08)',
                        display:'flex', flexDirection:'row'
                    }}>
                        {/* <span>{Moment(item.start).format('ddd, D MMM HH.mm')}</span>
                        ...
                        <span>{Moment(item.start).format('ddd, D MMM HH.mm')} - {Moment(item.end).format('HH.mm')}</span> */}
                        ...
                        <div style={{display:'flex', flexDirection:'column', marginLeft: 21, width: '100%'}}>
                            ...
                            <span style={{height: 1.5, width: '100%', backgroundColor: 'rgba(0,0,0,0.03)', marginTop:10}}></span>
    
                            <div style={{display:'flex', direction:'row', flexWrap: 'wrap', marginTop: 10,}}>
                                {/* <span style={{marginTop: 8, fontSize: 16}}>{Moment(item.start).format('Do, MMM h.mm a')}  -  {Moment(item.end).format('Do, MMM h.mm a')}</span> */}
                                <span style={{marginTop: 0, fontSize: 12, marginRight: 10, color :'rgba(0,0,0,0.5)'}}>{Moment(item.start).format('h.mm a')}  -  {Moment(item.end).format('h.mm a')}</span>
    
                                <div style={{marginTop: 0, display:'flex', justifyContent:'flex-start', flexWrap: 'wrap'}}>
    
                                    {new Date() > new Date(item.start) &&
                                        <span style={{marginRight: 5, fontSize: 12, color:'rgba(0,0,0,0.5)'}}>Started {Moment(item.start).fromNow()}</span>
                                    }
                                    {new Date() < new Date(item.start) &&
                                        <span style={{marginRight: 5, fontSize: 12, color:'rgba(0,0,0,0.5)'}}>Starting {Moment(item.start).fromNow()}</span>
                                    }
    
                                    <span style={{marginRight: 5, fontSize: 12, color:'rgba(0,0,0,0.5)'}}>|</span>
                                    {/* {new Date() > new Date(item.end) &&
                                        <span style={{marginRight: 15}}>Ended {Moment(item.end).fromNow()}</span>
                                    } */}
                                    {new Date() < new Date(item.end) &&
                                        <span style={{marginRight: 5, fontSize: 12, color:'rgba(0,0,0,0.5)'}}>Ending {Moment(item.end).fromNow()}</span>
                                    }
            ...
            EventList = (
                <div style={{
                    ...
                    <span style={{fontSize: 16, fontWeight: '500'}}>No upcoming events</span>
                </div>
            )
        }
        ...
        return(
            <div>
                ...
                <div style={{marginBottom: 20, marginTop: 25, display:'flex', flexDirection:'row', alignItems:'center'}}>
                    <span style={{fontSize: 16, fontWeight: '500', marginRight: 20}}>UPCOMING EVENTS</span>
